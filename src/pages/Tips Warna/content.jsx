...
import { ChipView, Heading, Img, Text, TipsCard } from "../../components";
...
export default function Content() {
    const [ChipOptions, setChipOptions] = React.useState(() => [
        { value: 1, label: `Kenali Undertone Kulit` },
        { value: 2, label: `Gunakan Kombinasi Monokromatik` },
        { value: 3, label: `Bereksperimen Dengan Motif` },
        { value: 4, label: `Pelajari Warna Analog` },
    ]);
                    ...
                    <div className="flex items-center justify-between gap-10 border border-solid border-black bg-white-a700 md:flex-row mb-[66px] ">
                        ...
                        <div className="flex flex-col items-start gap-11 md:px-5"> {/* Kontainer teks dan ChipView */}
                            <div className="flex flex-col items-start gap-11"> {/* Elemen teks */}
                                <Heading as="h2" className="text-[48px] font-bold text-blk md:text-[44px] sm:text-[38px]">
                                    Tips Memilih Bahan
                                </Heading>
                                <Text as="p" className="w-[84%] text-[18px] font-normal leading-7 text-blk md:w-full">
                                    ...
                                    <React.Fragment key={option.index}>
                                        {option.isSelected ? (
                                            <div
                                                ...
                                                className=
                                                "flex h-[32px] min-w-[186px] cursor-pointer flex-row items-center justify-center border border-solid border-blk border-white-a700_99 bg-white-a700 px-2.5 text-center text-[18px] text-blk"
                                                >
                                            ...
                                            <div
                                            onClick={option.toggle}
                                            className="flex h-[32px] min-w-[186px] cursor-pointer flex-row items-center justify-center border border-solid border-blk bg-white-a700 px-2.5 text-center text-[18px] text-blk"
                                            >
                                                <span>{option.label}</span>
                                            </div>
                                        )}
